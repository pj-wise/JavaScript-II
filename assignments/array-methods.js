...
runners.forEach(function(runnerID){
    fullName.push(runnerID.first_name + " " + runnerID.last_name);
});
...
runners.map(runnerID => {
   allCaps.push(runnerID.first_name.toUpperCase());
});
...
let largeShirts = [];
largeShirts = runners.filter(runnerID => runnerID.shirt_size === 'L');
